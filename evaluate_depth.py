...
def save_error_visualization(gt,pred,i):
    gt = (gt - gt.min())/(gt.max() - gt.min())
    pred = (pred - pred.min())/(pred.max() - pred.min())
    diff_1 = np.abs(gt - pred)
    diff = (diff_1 - diff_1.min())/(diff_1.max() - diff_1.min())
    diff_rel = diff_1 / (gt + 0.03)
    diff_rel = (diff_rel - diff.min())/(diff_rel.max() - diff_rel.min())
    max_min_diff = get_value_range(diff)
    max_min_diff_rel = get_value_range(diff_rel)
    normalizer_diff = mpl.colors.Normalize(vmin = max_min_diff[0], vmax = max_min_diff[1])
    normalizer_diff_rel = mpl.colors.Normalize(vmin = max_min_diff_rel[0], vmax = max_min_diff_rel[1])
    mapper_diff = cm.ScalarMappable(norm=normalizer_diff, cmap='magma')
    mapper_diff_rel = cm.ScalarMappable(norm=normalizer_diff_rel, cmap='magma')
    colormapped_diff = (mapper_diff.to_rgba(diff)[:, :,:3] * 255).astype(np.uint8)
    colormapped_diff_rel = (mapper_diff_rel.to_rgba(diff_rel)[ :, :,:3] * 255).astype(np.uint8)
    im_diff = pil.fromarray(colormapped_diff)
    im_diff_rel = pil.fromarray(colormapped_diff_rel)
    diff_path = os.path.join('.','error_vis','{}th_diff.jpeg'.format(i))
    diff_rel_path = os.path.join('.','error_vis','{}th_diff_rel.jpeg'.format(i))
    im_diff.save(diff_path)
    im_diff_rel.save(diff_rel_path)
...
def compute_errors(gt, pred):
    ...
    #max_loss = np.abs(pred - gt)/np.abs(gt)
...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        print(opt.data_augment)
        dataset = datasets.KITTIRAWDataset(opt.data_path, filenames,
                                           encoder_dict['height'], encoder_dict['width'],
                                           [0], 4, is_train=False,data_augment = opt.data_augment)
